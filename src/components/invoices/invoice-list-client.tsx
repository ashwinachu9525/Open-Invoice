...
import { bulkDeleteInvoices, cloneInvoice } from "@/actions/invoice"
...
export function InvoiceListClient({ invoices, currentPage, limit, totalPages, totalInvoices, searchParamsProps }: InvoiceListClientProps) {
  ...
  const handleClone = async (id: string) => {
    setIsCloning(id)
    const res = await cloneInvoice(id)
    setIsCloning(null)
    if (res.error) {
      toast.error(res.error)
    } else {
      toast.success("Invoice cloned successfully")
      if (res.id) router.push(`/invoices/${res.id}/edit`)
    }
  }
    ...
    <>
      {selectedIds.length > 0 && (
        <div className="bg-primary/10 border border-primary/20 p-2 px-4 rounded-md flex items-center justify-between mb-4 animate-in fade-in slide-in-from-top-2">
          <span className="text-sm font-medium text-primary">{selectedIds.length} invoices selected</span>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will move the selected invoices to the trash. You can restore them later or permanently delete them from the trash page.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
